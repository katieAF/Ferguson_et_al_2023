...
% experiment information and state data (MI struct), sets experiment-specific figure properties, creates distribution plots 
...
%
% Dependencies:
%   - Requires data files 'VIP-SST_MI.mat' and 'VIP-PN_MI.mat', located in the cfg.dir.baseDir folder.
%
% Data structure:
%   - The loaded data files contain a struct "MI" with fields:
%       - MI.MI: state modulation index data
%       - MI.expType: 0 or 1 for control or caspase recording
%       - MI.cellID: information about the recordings
%       - MI.cellIDLabel: the label for each column of cellID
%       - MI.signif: logical indicating significance of modulation index
...
for iPop = 1:nExps
    ...
    cfg = setFigPropExp(cfg, infoSummary, iPop);

    f(iPop) = figure;
    set(f(iPop), 'Position', [500 250 725 700])
    
    % make histogram
    makeMIhistogram(cfg, MI, f(iPop), iPop);

    % make box/whisker plot
    makeMIboxplot(cfg, MI, f(iPop), iPop);
...
function cfg = setFigPropBasic(cfg)
% SETFIGPROPBASIC sets basic figure properties and stores in variable "cfg"
...
cfg.plt.boxrange = [-1,1]; % [-.5,.8];%

end


function cfg = setFigPropExp(cfg, infoSummary, iPop)
% SETFIGPROPEXP sets individual figure properties for each experiment, and stores in variable "cfg"

cfg.plt.expTypeStr{iPop} = infoSummary.expTypeFig;
cfg.plt.colors{iPop} = infoSummary.expTypeFigCol;
cfg.plt.fullTitle{iPop} = infoSummary.fullExpType;
cfg.plt.expType{iPop} = infoSummary.expType;

end

function makeMIhistogram(cfg, MI, f, popln)
% MAKEMIHISTOGRAM makes a histogram of the modulation indices, with
% significantly cells filled in
...
for iExp = 1:length(unExpType)
    ...
    col = cfg.plt.colors{popln}{iExp};
    expIdx = MI.expType == unExpType(iExp);
    titleStr = cfg.plt.expTypeStr{popln}{iExp};
...
suptitle(strrep(cfg.plt.fullTitle{popln}, '_', ' '));

end


function makeMIboxplot(cfg, MI, f, popln)
...
% set colors
col = vertcat(cfg.plt.colors{popln}{:});

% box plot 
plb = boxplot(MI.MI, MI.expType, ...
    'symbol','','Labels', cfg.plt.expType{popln}, 'Color', col);
